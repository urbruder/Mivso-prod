---
name: lockbox-access-log-tracker
description: "Use this skill whenever an agent needs a lockbox access log tracker, asks to build a lockbox access log tracker, or needs help with this catalog job: Tracks who entered a property and when, with a clean audit log. Produces an Excel workbook for Lockbox Access Log Tracker."
---
...
Creates a finished Excel workbook for Lockbox Access Log Tracker. It helps the agent handle this catalog job: Tracks who entered a property and when, with a clean audit log. The output is formatted so it can be reviewed, shared, printed, or used in a client, team, broker, investor, vendor, borrower, owner, or title conversation without another formatting pass.

## How to Run This Skill
1. Tell Claude: "Build a lockbox access log tracker"
...
## Output Format
An Excel workbook named Lockbox_Access_Log_Tracker_with_date.xlsx with five polished sheets: (1) Dashboard - a one-page visual summary with key decisions, risk flags, totals, and next actions; (2) Source Inputs - the facts, documents, assumptions, and notes supplied by the agent; (3) Analysis Matrix - the main comparison, checklist, tracker, scoring table, or calculation model for Lockbox Access Log Tracker; (4) Action Plan - owner, due date, priority, status, and follow-up columns; (5) Client or Team Notes - plain-English talking points and verification reminders. Format with frozen headers, readable column widths, wrapped notes, status labels, and a print-ready summary area.
...
7. Convert the source material into a polished Excel workbook that directly supports this job: Tracks who entered a property and when, with a clean audit log.
...
- "Build a lockbox access log tracker"
- "Create a client-ready lockbox access log tracker"
- "Turn these notes into a lockbox access log tracker"
- "Help me prepare a lockbox access log tracker for my next real estate conversation"
- "I need a polished lockbox access log tracker deliverable"
