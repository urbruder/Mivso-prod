---
name: mls-idx-usage-rules-guide
description: "Use this skill whenever an agent needs a mls and idx usage rules guide, asks to build a mls and idx usage rules guide, or needs help with this catalog job: Keeps website display, attribution, and refresh within each MLS's rules. Produces an Excel workbook for MLS / IDX Usage Rules Guide."
---
...
Creates a finished Excel workbook for MLS / IDX Usage Rules Guide. It helps the agent handle this catalog job: Keeps website display, attribution, and refresh within each MLS's rules. The output is formatted so it can be reviewed, shared, printed, or used in a client, team, broker, investor, vendor, borrower, owner, or title conversation without another formatting pass.

## How to Run This Skill
1. Tell Claude: "Build a mls and idx usage rules guide"
...
## Output Format
An Excel workbook named MLS_IDX_Usage_Rules_Guide_with_date.xlsx with five polished sheets: (1) Dashboard - a one-page visual summary with key decisions, risk flags, totals, and next actions; (2) Source Inputs - the facts, documents, assumptions, and notes supplied by the agent; (3) Analysis Matrix - the main comparison, checklist, tracker, scoring table, or calculation model for MLS / IDX Usage Rules Guide; (4) Action Plan - owner, due date, priority, status, and follow-up columns; (5) Client or Team Notes - plain-English talking points and verification reminders. Format with frozen headers, readable column widths, wrapped notes, status labels, and a print-ready summary area.
...
7. Convert the source material into a polished Excel workbook that directly supports this job: Keeps website display, attribution, and refresh within each MLS's rules.
...
- "Build a mls and idx usage rules guide"
- "Create a client-ready mls and idx usage rules guide"
- "Turn these notes into a mls and idx usage rules guide"
- "Help me prepare a mls and idx usage rules guide for my next real estate conversation"
- "I need a polished mls and idx usage rules guide deliverable"
